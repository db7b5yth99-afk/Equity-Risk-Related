...
# 0. Data fetching
def get_price(tickers, decision_date, window=30):
    if not isinstance(decision_date, pd.Timestamp):
        decision_date = pd.Timestamp(decision_date)
    hist_start = decision_date - pd.Timedelta(days=window + 10)
    end = decision_date
    if hist_start < pd.Timestamp('1990-01-01') or end > pd.Timestamp.now() + pd.Timedelta(days=1):
        return None
    try:
        data = yf.download(tickers, start=hist_start, end=end, progress=False, auto_adjust=True)['Close']
...
def find_transaction_loss(current_weight, target_weight, fee_proportion=0.001):
    l1 = np.linalg.norm(current_weight - target_weight, ord=1)
...
# Main simulation
def run_tsang_sim(tickers, start_date, total_days=60, window=30,
    ...
    for t in range(total_days):
        data = get_price(tickers, start, window)
        ...
        next_data = get_price(tickers, next_decision, window=5)
...
if __name__ == "__main__":
    ...
    print("\nNote: These are standalone expert performances. For the full adaptive RELP-Adap strategy, implement the SB / Top-K switching logic from the paper.")
